...
// kvmem-webui.h removed - using only stock llama.cpp webui
...
#if defined(KVMEM_HAVE_LLAMA_UI)
// Generated by tools/ui/embed.cpp; exposes the stock llama.cpp Web UI assets.
#include "ui.h"
#endif
...
// Serve the stock llama.cpp Web UI (tools/ui) when this build embedded its
// assets. Mirrors tools/server/server-http.cpp so the page, its hashed
// bundles, the service worker and the PWA manifest all resolve the same way.
// Returns false when there is nothing to serve, letting the caller fall back.
static bool kvmem_serve_upstream_ui(httplib::Server & svr) {
#if defined(KVMEM_HAVE_LLAMA_UI) && defined(LLAMA_UI_HAS_ASSETS)
    static auto handle_gzip_header = [](const httplib::Request & req, httplib::Response & res) {
        if (!llama_ui_use_gzip()) {
            return true; // no gzip build, skip
        }
    ...
    // Hashed assets are immutable; index.html names them, so it must revalidate.
    ...
    auto serve_asset_cached = [](const std::string & name, bool isolation, const char * cache_control) {
        return [name, isolation, cache_control](const httplib::Request & req, httplib::Response & res) {
            if (!handle_gzip_header(req, res)) {
                return true; // already produced an error response
            }
            ...
            if (!inm.empty() && (inm == a->etag || inm == std::string("W/") + a->etag)) {
                res.status = 304;
                return false;
            }
            if (isolation) {
    ...
    svr.Get("/",           serve_asset_cached("index.html", true, cache_revalidate));
    svr.Get("/index.html", serve_asset_cached("index.html", true, cache_revalidate));

    // PWA revalidation files must not be cached; everything else is immutable.
    ...
    for (const auto & a : llama_ui_get_assets()) {
        if (a.name == "index.html") continue; // served above
        if (no_cache_names.count(a.name)) {
            svr.Get("/" + a.name, serve_asset_nocache(a.name));
        } else {
            svr.Get("/" + a.name, serve_asset_cached(a.name, false, cache_immutable));
        }
    }
    return true;
#else
...
int main(int argc, char ** argv) {
    ...
    int n_ctx = 2048;
    int ngl = 99;
    // How the weights are loaded (upstream llama.cpp `-lm/--load-mode`).
    // LLAMA_LOAD_MODE_AUTO is upstream's default: pick based on device capability.
    llama_load_mode load_mode = LLAMA_LOAD_MODE_AUTO;
    // Flash Attention (upstream llama.cpp `-fa/--flash-attn on|off|auto`).
    // AUTO is upstream's default. Note that a quantized V cache (--kv-dtype
    // q8_0/q4_0) hard-requires FA, so `-fa off` only works with f16 K/V.
    llama_flash_attn_type flash_attn_type = LLAMA_FLASH_ATTN_TYPE_AUTO;
    // Served page: built-in chat UI at "/", or a directory of static files.
    bool webui = true;
    std::string public_path;
    bool server_browser = false;
    ServerState st;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (eq(arg, "-h") || eq(arg, "--help")) {
            ...
            // Mirrors upstream llama.cpp `-lm/--load-mode`.
            const char * t = need(arg);
            /**/ if (eq(t, "auto"))       { load_mode = LLAMA_LOAD_MODE_AUTO;       }
            else if (eq(t, "none"))       { load_mode = LLAMA_LOAD_MODE_NONE;       }
            else if (eq(t, "mmap"))       { load_mode = LLAMA_LOAD_MODE_MMAP;       }
            else if (eq(t, "mlock"))      { load_mode = LLAMA_LOAD_MODE_MLOCK;      }
            else if (eq(t, "mmap+mlock")) { load_mode = LLAMA_LOAD_MODE_MMAP_MLOCK; }
            else if (eq(t, "dio"))        { load_mode = LLAMA_LOAD_MODE_DIRECT_IO;  }
            else {
                fprintf(stderr, "invalid %s %s (want auto|none|mmap|mlock|mmap+mlock|dio)\n", arg, t);
                return 1;
            }
        } else if (eq(arg, "--mlock")) {
            // Deprecated upstream spelling, kept for CLI compatibility.
            fprintf(stderr, "DEPRECATED: --mlock is deprecated. use --load-mode mlock instead\n");
            load_mode = LLAMA_LOAD_MODE_MLOCK;
        } else if (eq(arg, "--mmap") || eq(arg, "--no-mmap")) {
            // Deprecated upstream spelling, kept for CLI compatibility.
            const bool on = eq(arg, "--mmap");
            fprintf(stderr, "DEPRECATED: --mmap/--no-mmap are deprecated. use --load-mode %s instead\n",
                    on ? "mmap" : "none");
            load_mode = on ? LLAMA_LOAD_MODE_MMAP : LLAMA_LOAD_MODE_NONE;
        } else if (eq(arg, "--direct-io") || eq(arg, "--no-direct-io")) {
            // Deprecated upstream spelling, kept for CLI compatibility.
            const bool on = eq(arg, "--direct-io");
            fprintf(stderr, "DEPRECATED: --direct-io/--no-direct-io are deprecated. use --load-mode %s instead\n",
                    on ? "dio" : "none");
            load_mode = on ? LLAMA_LOAD_MODE_DIRECT_IO : LLAMA_LOAD_MODE_NONE;
        } else if (eq(arg, "-fa") || eq(arg, "--flash-attn")) {
            // Mirrors upstream llama.cpp `-fa/--flash-attn on|off|auto`.
            const char * t = need(arg);
            /**/ if (eq(t, "on"))   { flash_attn_type = LLAMA_FLASH_ATTN_TYPE_ENABLED;  }
            else if (eq(t, "off"))  { flash_attn_type = LLAMA_FLASH_ATTN_TYPE_DISABLED; }
            else if (eq(t, "auto")) { flash_attn_type = LLAMA_FLASH_ATTN_TYPE_AUTO;     }
            else {
                fprintf(stderr, "invalid %s %s (want on|off|auto)\n", arg, t);
                return 1;
            }
        } else if (eq(arg, "--webui")) {
            webui = true;
        } else if (eq(arg, "--no-webui")) {
            webui = false;
        } else if (eq(arg, "--path")) {
            public_path = need(arg);
            webui = true;
        } else if (eq(arg, "--server-browser")) {
            server_browser = true;
        } else if (eq(arg, "--no-server-browser")) {
    ...
    cparams.n_seq_max = 1;
    cparams.flash_attn_type = flash_attn_type;
    ...
    const std::string build_info =
        std::string("kvmem-llama.cpp / llama.cpp ") + llama_version();

    // Server properties the stock llama.cpp Web UI (tools/ui) probes on load.
    // Without this it greys out with "Server unavailable". Fields the UI only
    // uses for display are filled from the live configuration where cheap; the
    // rest get neutral values so the UI never trips on a missing key.
    svr.Get("/props", [&, build_info](const httplib::Request &, httplib::Response & res) {
        auto sp = kvmem_chat_sampling_defaults(st.enable_thinking_default);
        std::string ignore_err;
        (void) kvmem_chat_sampling_override(st.sampling_overrides, sp, ignore_err);

        std::string tmpl;
        {
            std::vector<char> buf(65536, 0);
            const int n = llama_model_meta_val_str(
                st.model, "tokenizer.chat_template", buf.data(), (int) buf.size());
            if (n > 0) tmpl.assign(buf.data(), (size_t) n - 1);
        }

        auto token_str = [&](llama_token tok) -> std::string {
            return tok == LLAMA_TOKEN_NULL ? std::string()
                                           : common_token_to_piece(st.ctx, tok);
        };
        ...
        json params = {
            ...
            {"mirostat_eta",         0.1},
            {"stop",                 json::array()},
            {"max_tokens",           st.n_predict_default},
            {"n_keep",               0},
            {"n_discard",            0},
            {"ignore_eos",           false},
            {"stream",               true},
            ...
            {"speculative.n_max",    st.spec_n_max},
            {"speculative.n_min",    0},
            {"speculative.p_min",    st.spec_p_min},
        ...
        json j = {
            ...
            {"total_slots", 1},
            {"model_path",  st.model_name},
            {"role",        "model"},
            {"modalities",  {
                {"vision", st.vision != nullptr},
                {"audio",  false},
                {"video",  false},
            }},
            {"chat_template", tmpl},
            {"bos_token",     token_str(llama_vocab_bos(st.vocab))},
            {"eos_token",     token_str(llama_vocab_eos(st.vocab))},
            ...
            res.set_chunked_content_provider("text/event-stream",
                [slot, &st, &req, toks, cid, request_id, max_tokens, sparams, parse_tools, formatted, stops,
                 spec_stream, ctx, vocab, make_emit_gen_wall](size_t, httplib::DataSink & sink) mutable {
                    ...
                    send(stream_choice_chunk(cid, json{{"role", "assistant"}}, nullptr, json(), st.model_name).dump());
                    ...
                    // Per-chunk timings consumed by the stock llama.cpp Web UI
                    // (tools/ui) to render tokens/s, token counts and duration
                    // live under each assistant message. Mirrors llama-server:
                    // prompt_* from prefill, predicted_* as a running tally.
                    auto build_timings = [&]() -> json {
                        const double gen_ms = std::chrono::duration<double, std::milli>(
                            std::chrono::steady_clock::now() - t_pf1).count();
                        return json{
                            {"prompt_n",    (int) toks.size()},
                            {"prompt_ms",   prefill_ms},
                            {"predicted_n", (int) gen.size()},
                            {"predicted_ms", gen_ms},
                            {"cache_n",     n_cache_hit},
                        };
                    };
                    if (spec_stream) {
                        const auto gst = kvmem_spec_generate(st.ctx, st.model, st.spec, toks, max_tokens, sparams,
                            [&](llama_token id, const std::string & piece, bool) {
                                ...
                                for (const auto & delta : sco.set_text(content, true)) {
                                    send(stream_choice_chunk(cid, delta, nullptr, build_timings(), st.model_name).dump());
                                }
                            },
                        ...
                        send(stream_choice_chunk(cid, json::object(), finish, build_timings(), st.model_name).dump());
                        send(stream_usage_chunk(cid, (int) toks.size(), (int) gen.size(), n_cache_hit, build_timings()).dump());
                        sink.write("data: [DONE]\n\n", 14);
                        multimodal_finish_request(st);
                        slot->unlock();
                        sink.done();
    ...
    if (webui) {
        if (!public_path.empty()) {
            // Upstream-compatible: serve a directory of static files.
            svr.set_mount_point("/", public_path);
        } else {
            // Use stock llama.cpp webui only
            kvmem_serve_upstream_ui(svr);
        }
    }

    std::string url = "http://" + host + ":" + std::to_string(port) + "/";
    if (server_browser && webui) {
        // svr.listen() blocks, so hand the browser launch to a background
        // thread that waits for the listener to come up first.
        std::thread([url]() {
            std::this_thread::sleep_for(std::chrono::milliseconds(800));
#ifdef _WIN32
            ShellExecuteA(nullptr, "open", url.c_str(), nullptr, nullptr, SW_SHOWNORMAL);
#elif defined(__APPLE__)
            (void) system(("open \"" + url + "\"").c_str());
#else
            (void) system(("xdg-open \"" + url + "\"").c_str());
#endif
        }).detach();
    }
    ...
    fprintf(stderr, "  flash-attn=%s  load-mode=%s  webui=%s  url=%s\n",
            llama_flash_attn_type_name(flash_attn_type), llama_load_mode_name(load_mode),
            webui ? (public_path.empty() ? "built-in" : public_path.c_str()) : "off", url.c_str());
