...
#ifdef _WIN32
// MSVC: POSIX positional-I/O headers are unavailable; the stub class below
// keeps the NvmeKvTier API but reports enabled() == false.
#include <direct.h>
#else
#include <sys/stat.h>
#include <sys/types.h>
...
#ifdef _WIN32
// ---------------------------------------------------------------------------
// Windows stub tier. The POSIX arena (pwrite/pread/fallocate/unlink-on-close)
// is not ported; NVMe offload is likewise not part of the product path, and
// every caller gates tier use on enabled(). The stub preserves the API so the
// runtime compiles unchanged and simply never spills to NVMe on Windows.
// ---------------------------------------------------------------------------
class NvmeKvTier {
public:
    explicit NvmeKvTier(NvmeKvTierConfig cfg) : cfg_(std::move(cfg)) {
        std::fprintf(stderr,
                     "[kvmem-io] nvme_tier=disabled platform=windows "
                     "action=cpu-tier-only\n");
    }
    ...
    void mark_present_range(uint32_t, uint32_t) {
        throw std::runtime_error("NVMe KV tier is disabled on this platform");
    }
    ...
    NvmeSlotPlacement place_block(uint32_t) { return {}; }
    NvmeSlotPlacement place_block_evicting(uint32_t) { return {}; }
    ...
    void write_spans(const std::vector<NvmeIoSpan> &, const void *, uint64_t,
                     NvmeBatchIoStats * = nullptr) const {
        throw std::runtime_error("NVMe KV tier is disabled on this platform");
    }
